...
export function NFTCard({ nft, listing, onBuy }: { nft: NFTMeta; listing?: Listing; onBuy?: () => void }) {
  ...
  return (
    <div className="group relative rounded-2xl overflow-hidden flex flex-col bg-card border border-border/60 shadow-md hover:shadow-2xl hover:-translate-y-1 hover:border-primary/50 transition-all duration-300">
      {/* gradient halo on hover */}
      <div className="pointer-events-none absolute -inset-px rounded-2xl opacity-0 group-hover:opacity-100 transition-opacity bg-gradient-to-br from-fuchsia-500/20 via-transparent to-pink-500/20" />
      ...
      <Link to="/marketplace/$id" params={{ id }} className="relative aspect-square overflow-hidden bg-gradient-to-br from-accent/40 to-secondary/40 block">
        ...
        {/* price ribbon */}
        {listing && (
          <span className="absolute bottom-3 left-3 px-2.5 py-1 rounded-full text-xs font-bold bg-gradient-to-r from-fuchsia-500 to-pink-500 text-white shadow-lg">
